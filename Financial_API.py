# ==========================================
# 📂 檔案名稱： Financial_API.py (迎接3月營收升級版 - 單行連貫+防斷行版)
# 💡 更新內容： 
#    1. 籌碼徽章移至「營收 M/Y%」正下方，並強制同一行顯示不換行。
#    2. 移除所有置頂的多餘徽章，讓基本面數據清晰可見。
#    3. 加上長字串防斷行保護，解決 GitHub 編輯器換行報錯。
# ==========================================
        ...
        if "df_vip" in st.session_state:
            ...
            if df is not None and not df.empty:
                ...
                with c1:
                    ...
                    # 🌟 防斷行保護機制：將過長的字串拆成多段安全組合
                    safe_markdown_str = (
                        f"📈 業外佔比: {row['最新業外佔比(%)']:.2f}% &nbsp;｜&nbsp; "
                        f"📉 合約負債: {row['最新季度流動合約負債(億)']:.2f}億 "
                        f"({row['最新季度流動合約負債季增(%)']:.2f}%)"
                    )
                    st.markdown(safe_markdown_str)
